...
# Eculidian Distance formula to find matching percentage
...
# # statistical model to calculate the matching percentage
# def calculate_matching_percentage(landmarks1, landmarks2, threshold):
#     if landmarks1 is None or landmarks2 is None:
#         return 0
#     num_landmarks = min(len(landmarks1.landmark), len(landmarks2.landmark))
#     distances = []
...
#         landmark1 = landmarks1.landmark[i]
#         landmark2 = landmarks2.landmark[i]
#         distance = ((landmark1.x - landmark2.x) ** 2 + (landmark1.y - landmark2.y) ** 2 + (landmark1.z - landmark2.z) ** 2) ** 0.5
#         distances.append(distance)
#     mean_distance = np.mean(distances)
#     std_distance = np.std(distances)
#     z_scores = [(distance - mean_distance) / std_distance for distance in distances]
#     matching_accuracy = sum([1 for z_score in z_scores if z_score < threshold]) / num_landmarks * 100
#     return matching_accuracy

cap1 = cv2.VideoCapture('a1.MP4') # put the newbie video here
cap2 = cv2.VideoCapture('a2.mp4') # put the expert video here
